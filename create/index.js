...
const templates = {
  components: {
    default: './create/templates/component/IExample.vue',
  },
  ...
  modules: {
    ...
    locales: [
      './create/templates/module/locales/example.en.yaml',
      './create/templates/module/locales/example.zh.yaml'
    ],
...
const createFolder = (type, folder) => {
  ...
  switch (type) {
    case 'component':
      path = `./src/components/${folder}/${camelCase(`i-${name}`, { pascalCase: true })}`
...
const createModule = {
  ...
  locales: () => {
    ...
    if (!checkPath(path)) {
      ...
      for (const index in templates.modules.locales) {
        shell.exec(`cat ${templates.modules.locales[index]} > ${path[index]}`)
...
program
  ...
  .description('command to generate component:[atoms](原子组件) [molecule](分子组件) [templates](模版组件)')
  .action((type, name) => {
